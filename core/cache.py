...
class CacheDB:
    ...
    def _ensure_open(self) -> None:
        if self._closed:
            raise RuntimeError("CacheDB is closed")

    def _write_with_rollback(self, sql, params=None):
        try:
    ...
    def _upsert_memory_record(self, record: dict) -> None:
        record_copy = dict(record)
        self._mem_by_qhash[record_copy["question_hash"]] = record_copy

        self._mem_phash_list = [
            row for row in self._mem_phash_list
            if row.get("question_hash") != record_copy["question_hash"]
        ]
        if record_copy.get("phash"):
            self._mem_phash_list.append(record_copy)

    def _select_record_by_question_hash(self, question_hash: str) -> Optional[dict]:
        cursor = self._conn.execute(
            """
            SELECT id, question_hash, phash, answer, source, answered, created_at, updated_at
            FROM cache
            WHERE question_hash = ?
            """,
            (question_hash,),
    ...
    def insert(
        ...
        with self._lock:
            ...
            record = self._select_record_by_question_hash(question_hash)
            if record is None:
                raise RuntimeError(f"缓存记录写入后未找到: {question_hash}")
            self._upsert_memory_record(record)
